...
var removeCmd = &cobra.Command{
	Use:   "delete",
	Short: "Deletes a given stock",
	Run: func(cmd *cobra.Command, args []string) {
		database.DeleteStock(portfolio, ticker)
	},
	Example: `budgie stock delete
	--portfolio "European Stocks"
	--ticker "MSFT"
`,
}
...
func init() {
	...
	stockCmd.AddCommand(addCmd, deleteCmd, reportCmd)
	...
	deleteCmd.PersistentFlags().StringVarP(&portfolio, "portfolio", "p", "", "Portfolio name (required)")
	deleteCmd.MarkPersistentFlagRequired("portfolio")
	deleteCmd.PersistentFlags().StringVarP(&ticker, "ticker", "s", "", "Company name (required)")
	deleteCmd.MarkPersistentFlagRequired("ticker")
